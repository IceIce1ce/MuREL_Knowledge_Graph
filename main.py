...
class Experiment:
    def __init__(self, learning_rate=0.001, dim=32, nneg=50, model="poineuclidean",
                 num_iterations=200, batch_size=128, cuda=False, optimization="Adam"):
        self.model = model
    ...
    def train_and_eval(self):
        ...
        if self.model.lower() == "murel":
            model = MuREL(d, self.dim)
        elif self.model.lower() == "rotel":
            model = RotEL(d, self.dim)
        else:
            raise Exception("This model isn't exist")
        ...
        if self.optimization == "Adam":
            ...
            print("Training with Adagrad optimization")
            opt = torch.optim.Adagrad(model.parameters(), lr=self.learning_rate)
        if self.cuda:
            model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--opt", type=str, default="Adam", nargs="?",
                    help="Which optimization to use: Adam or Adagrad.")
